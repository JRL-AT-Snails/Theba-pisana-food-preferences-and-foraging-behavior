---
title: |
 ![](C:\Users\Utilisateur\Snails_Project\Temoins){width=2in} third pre-treatment with Savitzky Golay rchemo and PLSDA
author: "Cing Lee, Romeo Roux-Vaneph, Adele Hardy, Aubin Patouillard"
...
plotsp(subset_data_diet, lwd = 2,
       col = c("green","brown"),
       main = "Mean spectra of faeces from the three sizes of control snails, either fed with only green leaves or decaying ones",
...
medianReflectance <- apply(subset_data_size, 1, median)

# Plot the subset of data using plotsp
plotsp(subset_data_size, lwd = 2,
       col = c("red","violet","orange"),
       main = "Mean spectra of faeces from control snails with the same shell size",
       xlab = "Wavelengths", ylab = "Reflectance")
# Add legend
legend("topright", legend = c("S1 : Large", "S2 : Medium","S3 : Small"), col = c("red","violet","orange"), lty = 1,lwd = 2)
```
...
```{r sampling}
  ...
  variety_val= matable$group[id_val]; variety_cal=matable$group[-id_val]
```
...
```{r calibplsda}
  ## PLSDA
# Initialization
  ncmax=10 #Fixe le nombre maximal de composantes principales à considérer dans la boucle à 10.
...
To set a more robust calibration, the cross-validation is repeated 50 times for each component value between 1 and `ncomp` in the `perf` function. 

```{r plsda,fig.width=12, fig.height=4}
plsda.res <- plsda(X=matable_cal, Y=variety_cal, ncomp = 5)
...
# using perf.plsda.res$choice.ncomp for Mahalanobis distance
  rplsda_final=plsda(matable_cal, variety_cal, ncomp=5)
# Performance
...
save_path <- paste0(data.dir,"/significant_results.csv")
write.csv(final_matrix,save_path)
...
plotIndiv(rplsda_final,comp=c(1,3),ellipse = TRUE,
          group = rplsda_final$Y,pch=as.numeric(matable_cal_group_size[,218]),
          pch.levels= ifelse(matable_cal_group_size[,218]==1,"large",ifelse(matable_cal_group_size[,218]==2,"medium","small")) ,legend=TRUE,col=c("brown","green"))
...
save_path <- paste0(data.dir,"/matrix_wild.csv")
write.csv(matrix_wild,save_path)
save_path <- paste0(data.dir,"/pred_wild.csv")
write.csv(pred_wild$variates,save_path)
save_path <- paste0(data.dir,"/rplsda.final$group_size.csv")
write.csv(matable_cal_group_size,save_path)
save_path <- paste0(data.dir,"/rplsda.final$coordonnees.csv")
write.csv(rplsda_final$variates$X,save_path)
```
